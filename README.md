...
| 역할을 분리한다 | `design-writer` · `code-writer` · `architecture-reviewer` 서브에이전트 | 설계·구현·검토를 독립된 인스턴스가 담당해 편향을 제거한다 |
| 루프를 돌린다 | `implement` 스킬 | D(설계) → A(구현) → B(검토) → 수정을 PASS까지 자동 수렴 |
...
```
claude-code-playbook/
├── .claude/                       # AI 에이전트 가드레일 (서브에이전트, 스킬, 훅, 권한)
