...
## 4. 첫 작업 — `/implement` 커맨드 체험

슬래시 커맨드로 기능 구현을 요청한다:

```
/implement "테넌트 온보딩 API를 추가해줘"
```
...
1. `docs/backend/` 하위 모든 문서에서 **예시 도메인 이름**(Product, Category, Tenant 등)을 자신의 도메인으로 치환.
2. `.claude/CLAUDE.md`의 "비즈니스 목표" 섹션을 자신의 프로젝트 설명으로 재작성.
3. 필요 없는 가이드라인은 삭제, 부족한 것은 추가.
...
|------|------|
| `claude` | 세션 시작 |
| `/implement <요구사항>` | 요구사항을 마일스톤으로 분해해 구현 |
| `/help` | Claude Code 도움말 |
| `/model` | 사용 모델 변경 |

프로젝트별 커스텀 커맨드는 `.claude/commands/` 하위에서 확인.
