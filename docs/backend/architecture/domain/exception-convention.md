...
- 도메인은 HTTP를 모른다. `CoreErrorType`으로 의미론적 분류를 표현하고, HTTP 상태 코드 매핑은 app 계층에 맡긴다.
- 예외 계층은 도메인별로 소유한다. 여러 도메인이 공유하는 범용 enum은 소속을 모호하게 만든다.
- 예외를 표현하는 방식은 용도에 따라 다르다. 프로그래머 실수(전제조건 위반)는 `require`/`check`로, 비즈니스 규칙 위반은 `CoreException`으로 표현한다.
- 새 예외를 추가해도 app 계층 수정이 필요 없다. `ErrorCode` 계약 기반 단일 핸들러가 자동으로 처리한다.
