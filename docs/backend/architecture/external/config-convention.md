...
- Provider별 HTTP 클라이언트는 분리한다. 타임아웃·인터셉터·baseUrl이 서로 간섭하면 한 Provider의 설정 변경이 다른 Provider에 영향을 미친다.
- 설정값은 코드에 하드코딩하지 않는다. 환경별로 달라지는 baseUrl·타임아웃을 외부 구성으로 주입해 배포 환경 전환을 코드 변경 없이 처리한다.
- 타임아웃은 항상 설정한다. 무한 대기는 장애 시 스레드 풀을 고갈시킨다.
- 민감 정보는 기본값에 두지 않는다. apiKey 같은 인증 정보는 운영에서 환경 변수로 주입하고, 기본값은 빈 문자열로 유지한다.
