...
- 외부 에러코드는 enum으로 중앙 관리한다. 문자열 직접 비교가 Adapter에 분산되면 외부 API 변경 시 누락이 발생한다.
- 번역은 단일 지점에서 수행한다. `toPortErrorCode()`가 외부→내부 매핑을 담당하고, Adapter는 enum만 참조한다.
- 컴파일 시점에 누락을 감지한다. `when`에서 `else` 없이 모든 enum 항목을 명시하면 신규 에러코드 추가 시 컴파일 오류로 알 수 있다.
