...
- 예외 처리는 관심사 분리의 연장이다. domain 계층은 HTTP를 모르고, app 계층이 HTTP 상태 코드 매핑 책임을 전담한다.
- 단일 진입점이 일관성을 보장한다. 분산된 `try-catch`는 처리 방식을 불일관하게 만들고 누락을 유발한다.
- 클라이언트에 내부 구현을 노출하지 않는다. 예외 원문 메시지가 아닌 `ErrorCode.message`로 클라이언트 응답을 제어한다.
- 로그 수준은 예외 영향도와 일치시킨다. 예상된 클라이언트 오류(4xx)는 WARN, 내부 오류(5xx)는 ERROR로 구분한다.
...
> 도메인 예외 계층(`CoreException`, `CoreErrorType`, `ErrorCode`, `{Domain}ErrorCode`) 상세는 [../domain/exception-convention.md](../domain/exception-convention.md) 참고
...
- 도메인 규칙 위반 → **금지** — `{Domain}ErrorCode`에 추가 ([../domain/exception-convention.md](../domain/exception-convention.md) 참고)
