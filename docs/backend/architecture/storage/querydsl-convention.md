...
- 쿼리 의도는 코드에서 읽힌다. `BooleanExpression?` 방식과 고정된 작성 순서는 쿼리를 선언적으로 읽히게 한다.
- 메모리 페이징은 시스템을 위협한다. fetchJoin + 페이지네이션은 대량 데이터 시 OOM을 유발한다. 2-step 조인은 이 문제를 인덱스 레벨에서 해결한다.
- 추출은 재사용이 확인된 후에 한다. 한 번만 쓰이는 조건을 메서드로 추출하면 불필요한 간접 계층이 생긴다.
- Projection은 타입 안전하게 수행한다. `Tuple.get()`은 컴파일 타임 타입 검증이 불가능하여 런타임 오류를 숨긴다.
