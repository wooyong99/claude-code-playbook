...
| 형태 | 문제 |
|------|------|
| 내부에서 Port를 주입받아 조회 수행 | 조회 책임 은폐 — Validator 핵심 규칙 위반 (ValidateFlow 안티패턴 재현) |
...
| 조건 | 퍼블릭 메서드 구성 | 예시 |
|------|------------------|------|
| 동일 입력만으로 판단되는 여러 규칙 | **하나의 메서드로 통합**, 세부 규칙은 내부 `private` 메서드로 분리 | `TermsRequiredValidator.validate` — 3개 Set 입력으로 "활성 버전 여부 + 필수 동의 여부"를 한 번에 검증 |
| 규칙 사이에 Port / Policy 조회가 끼어드는 경우 | **메서드 분리** — 호출 순서와 중간 조회가 UseCase/Flow 흐름에 드러나야 함 | `SlugValidator.validateFormat` → `slugReservedPolicy.isReserved()` 조회 → `SlugValidator.validateNotReserved` |
| 조건에 따라 일부 규칙만 적용되는 경우 | **메서드 분리** — 생성·수정 등 컨텍스트별 선택 가능 | 생성 시 모든 규칙 vs 수정 시 일부 규칙 |
